...
#include <mw/net/can/can_eg.h>
#include <mw/net/can/gen_can_mw.h>
    ...
    int32_t realtime_resolution)
{
    ...
    anet_lib_add_gen_can_mw(&s.loader, NULL);
    ...
    while ((alive || ascheduler_num_processes(&s) > 0) && (is_ctrl_c == false)) {
...
void consoleHandler(int sig){ // can be called asynchronously
    printf("Ctrl-C handled\n"); // do cleanup
    is_ctrl_c = true; // set flag
}
#else
#endif
